...
const Home = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/jewelery/get_all_jewelerys');
                    ...
                    {products.map((product) => (
                        <div key={product._id} className="product-item">
                            <Link to={`/product/${product._id}`}>
                                <img src={`http://localhost:5000/jewelerys/${product.jeweleryImage}`} alt={product.jeweleryName} />
